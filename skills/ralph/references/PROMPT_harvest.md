...
## Failure Prevention
- <pattern from journal> → <what to do instead>
- <pattern from journal> → <what to do instead>

## Project-Specific Commands
- <command or workflow that differs from AGENTS.md defaults>

## Time Budget Notes
- <observations about which task types need more/less time>
```

If `RALPH_OVERRIDES.md` already exists (from a previous run or human edits), **merge** your new findings into it. Do not delete existing rules unless the journal shows they are wrong. Append new rules, update outdated ones, and remove any that the current run disproved.
